...
export function SafetyChatWidget({
  ...
  const panel = (
    <div
      className={`chat-shell flex flex-col overflow-hidden rounded-[28px] border border-[#d7e2eb] bg-white/96 shadow-[0_24px_48px_rgba(15,23,42,0.12)] backdrop-blur-xl ${
        embedded ? "min-h-[62vh]" : "max-h-[min(68vh,520px)] w-[min(352px,calc(100vw-1.5rem))]"
      }`}
    >
      <div className="flex shrink-0 items-start justify-between gap-3 px-4 pt-4">
        <div>
          ...
          <h3 className="mt-1 font-display text-base font-semibold leading-snug">Smart local chat with profile-aware guidance</h3>
        </div>
        {!embedded ? (
          <button
      ...
      <div className="mx-4 mt-3 shrink-0 overflow-hidden rounded-[16px] bg-white/80 panel-outline">
        ...
        >
          ...
          {settingsOpen ? <ChevronUp size={13} className="shrink-0 text-ink/35" /> : <ChevronDown size={13} className="shrink-0 text-ink/35" />}
        </button>
          ...
          <div className="border-t border-[#eef1f5] px-3 pb-3 pt-2.5">
            ...
            <div className="grid grid-cols-2 gap-2">
              {productLensOptions.map((option) => {
                ...
                const description = option === "cosmetic" ? "Beauty & skin care." : "Food & pantry safety.";

                return (
                  <button
                    key={option}
                    onClick={() => setLens(option)}
                    className={`rounded-[12px] px-3 py-2.5 text-left ${active ? "bg-[#0f172a] text-white" : "bg-[#edf4f8] text-[#334155]"}`}
                  >
                    <p className="text-sm font-semibold">{title}</p>
                    <p className={`mt-0.5 text-xs ${active ? "text-white/65" : "text-ink/45"}`}>{description}</p>
                  </button>
                );
              })}
            </div>
            ...
            <div className="grid grid-cols-2 gap-2">
              <button
                ...
                className={`rounded-[12px] px-3 py-2.5 text-left ${chatScope === "selected" ? "bg-[#0f172a] text-white" : "bg-[#edf4f8] text-[#334155]"}`}
              >
                <p className="text-sm font-semibold">Selected</p>
                <p className={`mt-0.5 text-xs ${chatScope === "selected" ? "text-white/65" : "text-ink/45"}`}>Profiles on page.</p>
              </button>
              <button
                onClick={() => setChatScope("all")}
                className={`rounded-[12px] px-3 py-2.5 text-left ${chatScope === "all" ? "bg-[#2563eb] text-white" : "bg-[#edf4f8] text-[#334155]"}`}
              >
                <p className="text-sm font-semibold">All profiles</p>
                <p className={`mt-0.5 text-xs ${chatScope === "all" ? "text-white/70" : "text-ink/45"}`}>Whole account.</p>
              </button>
            </div>

            {chatScope === "selected" ? (
              <p className="mt-2 rounded-[10px] bg-[#f1f7fb] px-3 py-2 text-xs text-ink/55">
                {profileIds.length ? `${profileIds.length} profile${profileIds.length > 1 ? "s" : ""} selected on page.` : "No profiles selected yet."}
              </p>
            ) : null}
          </div>
        ) : null}
      ...
      <div className="mt-3 min-h-0 flex-1 space-y-2 overflow-y-auto px-4">
        {messages.map((message) => (
          <div
            key={message.id}
            className={`rounded-[16px] px-3 py-2.5 text-sm leading-[1.55] ${
              message.role === "assistant" ? "bg-white text-ink/75 panel-outline" : "ml-6 bg-[#0f172a] text-white"
            }`}
          >
            {message.text}
          </div>
      ...
      <div className="shrink-0 border-t border-[#e8eef3] px-4 pb-4 pt-3">
        ...
        <div className="flex items-end gap-2">
          <textarea
            ...
            rows={2}
            className="panel-outline min-h-[52px] flex-1 resize-none rounded-[16px] bg-white px-3 py-2.5 text-sm leading-5 outline-none"
          />
          <button
