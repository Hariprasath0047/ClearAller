...
export async function getOrCreateIngredientKnowledge(ingredient: string) {
  ...
  if (existing) {
    const existingHasCoverage =
      Boolean(existing.description && existing.description.length >= 12) ||
      (existing.categories?.length ?? 0) > 0 ||
      (existing.synonyms?.length ?? 0) > 1 ||
      (existing.derivatives?.length ?? 0) > 1;

    if (existingHasCoverage) {
      return existing;
    }
...
export async function hydrateIngredientKnowledge(ingredients: string[]) {
  return Promise.all(ingredients.map((ingredient) => hydrateLimit(() => getOrCreateIngredientKnowledge(ingredient))));
}
