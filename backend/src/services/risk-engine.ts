...
type AnalyzeResponse = {
  ingredients: string[];
  predictions: SafetyPredictionWithSignals[];
  ml: {
    pythonModelAvailable: boolean;
    pythonSignals: number;
...
function setCachedAnalysis(key: string, value: AnalyzeResponse) {
  analysisCache.set(key, { value, expiresAt: Date.now() + ANALYSIS_CACHE_TTL_MS });
}
...
export async function analyzeIngredients(params: {
  ...
  const cacheKey = JSON.stringify({
    ...
    extractedText: params.extractedText.trim().toLowerCase(),
    productQuery: params.productQuery?.trim().toLowerCase() ?? "",
    ...
    return {
      ...
      notes: [
        ...
        unknownIngredients.length
          ? `Additional ingredient review may be helpful for: ${unknownIngredients.slice(0, 5).join(", ")}${unknownIngredients.length > 5 ? ", ..." : ""}.`
          : "The extracted ingredients matched the current knowledge base for review.",
        "Always verify the label manually for safety-critical decisions."
      ],
